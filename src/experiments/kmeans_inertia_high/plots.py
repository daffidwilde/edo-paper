...
IMAGES = Path("../../../img/")
SUMMARY_DIR = Path(f"../../../data/kmeans_inertia_high/{SEED}/summary/")
...
def fitness_boxplot():
    ...
    plt.tight_layout()
    plt.savefig(IMAGES / "Fig7b-1.pdf", transparent=True)
...
def nrows_boxplot():
    ...
    plt.tight_layout()
    plt.savefig(IMAGES / "Fig7b-2.pdf", transparent=True)
...
def kmeans_scatterplot():
    ...
    plt.tight_layout()
    plt.savefig(IMAGES / f"Fig8b.pdf", transparent=True)
