""" Code to generate the plots for k-means with inertia and lower bounds. """
...
IMAGES = Path("../../../img/")
SUMMARY_DIR = Path(f"../../../data/kmeans_inertia/{SEED}/summary/")
...
def fitness_boxplot():
    ...
    ax.set_yscale("log")
    ax.set_xlabel("Epoch")
    ax.set_ylabel(r"Final inertia ($\log$)")

    plt.tight_layout()
    plt.savefig(IMAGES / "Fig7a-1.pdf", transparent=True)
...
def nrows_boxplot():
    ...
    plt.tight_layout()
    plt.savefig(IMAGES / "Fig7a-2.pdf", transparent=True)
...
def kmeans_scatterplot():
    ...
    plt.tight_layout()
    plt.savefig(IMAGES / f"Fig8a.pdf", transparent=True)
