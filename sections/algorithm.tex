...
\begin{itemize}
    ...
    \item Limits on the number of columns a dataset can have
        ...
        \]
        for each \(j = 1, \ldots, |\mathcal{P}|\). That is, \(C\) is defined as
        a tuple detailing the minimum and maximum number of columns a dataset
        can have for each distribution passed in \(\mathcal{P}\).
