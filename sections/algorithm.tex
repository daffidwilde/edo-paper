...
On the surface, the EDO method is built as an evolutionary algorithm which
follows a traditional (generic) schema with some additional features that keep
...
\begin{itemize}
    \item A real-values fitness function, \(f\), which acts on a single dataset
        to give a single fitness score.
    \item A population size, \(N \in \mathbb{N}\).
    \item A maximum number of iterations, \(M \in \mathbb{N}\).
    \item A selection parameter to detail the proportion of the fittest
        individuals to carry forward, \(b \in [0, 1]\).
...
The statement of the EDO algorithm is deliberately vague. This is, in part,
